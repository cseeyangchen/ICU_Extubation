...
class Model(nn.Module):
    def __init__(self, time, height, width, channel,
                 ...
                 num_class, dropout):
        super().__init__()
        self.dimension_reduction1 = DimensionReduction(in_channel=channel, out_channel=out_channel,kernel_size=kernel_size, stride=stride, padding=padding)
        self.dimension_reduction2 = DimensionReduction(in_channel=channel, out_channel=out_channel,
        ...
        # rgb and mask learning
        self.rgb_spatial = nn.Linear((out_time*height*width), (out_time * height * width))
        self.mask_spatial = nn.Linear((out_time * height * width), (out_time * height * width))
        self.rgb_temporal = nn.Linear((out_channel*height*width), (out_channel*height*width))
        self.mask_temporal = nn.Linear((out_channel * height * width), (out_channel * height * width))
        # fusion
    ...
    def forward(self, x1, x2, flag):
        if flag == 'train':
            # 对通道进行降维  256--8
            x1 = self.dimension_reduction1(x1)
            x2 = self.dimension_reduction2(x2)
            # 预处理操作 -- 添加cls_token和pos_embedding
            _, spatial_token1, temporal_token1 = self.token_process(x1)
            _, spatial_token2, temporal_token2 = self.token_process(x2)
            # rgb and mask learning
            spatial_token1 = self.rgb_spatial(spatial_token1)
            spatial_token2 = self.rgb_spatial(spatial_token2)
            # spatial_token2 = self.mask_spatial(spatial_token2)
            temporal_token1 = self.rgb_temporal(temporal_token1)
            temporal_token2 = self.rgb_temporal(temporal_token2)
            # temporal_token2 = self.mask_temporal(temporal_token2)
            spatial_token = torch.max(spatial_token1*10, spatial_token2)
            temporal_token = torch.max(temporal_token1*10, temporal_token2)
            # spatial Transformer
            # spatial_token = self.spatial_transformer(torch.cat((spatial_token1, spatial_token2), dim=1))
            spatial_token = self.spatial_transformer(spatial_token)
            # temporal Transformer
            # temporal_token = self.temporal_transformer(torch.cat((temporal_token1, temporal_token2), dim=1))
            ...
            temporal_token1 = self.rgb_temporal(temporal_token1)
            temporal_token2 = self.rgb_temporal(temporal_token2)
            spatial_token = torch.max(spatial_token1, spatial_token2*0.1)
            temporal_token = torch.max(temporal_token1, temporal_token2*0.1)
            # spatial Transformer
            # spatial_token = self.spatial_transformer(torch.cat((spatial_token1, spatial_token2), dim=1))
            spatial_token = self.spatial_transformer(spatial_token)
            # temporal Transformer
    ...
    def extract(self, x):
        ...
        cls_token = spatial_temporal_token[:,0]
        output = self.classifier(cls_token)
        return spatial_temporal_token[:,1:]
