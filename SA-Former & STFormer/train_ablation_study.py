...
from net.ablation_study.w_spatial_transformer import Model as w_smodel
from net.ablation_study.w_temporal_transformer import Model as w_tmodel
from net.ablation_study.w_spatial_temporal_transformer import Model as w_stmodel

from sklearn.model_selection import StratifiedKFold

def processor(dataset, ablation, optimizer_name, lr, weight_decay, device_list,
    ...
    for train, test in skf.split(filedata_names, label_array):
        ...
        # 加载模型
        if ablation=='w_spatial':
            model = w_smodel(time=32, height=8, width=8, channel=256,
        ...
        elif ablation=='w_temporal':
            model = w_tmodel(time=32, height=8, width=8, channel=256,
                          out_time=16, out_channel=8, kernel_size=(2, 1, 1), stride=(2, 1, 1), padding=(0, 0, 0),
                          head_spatial_dim=256, head_spatial_cnt=4, scale_spatial_dim=2,
                          head_temporal_dim=512, head_temporal_cnt=4, scale_temporal_dim=2,
                          st_dim=512, head_st_dim=128, head_st_cnt=4, scale_st_dim=2,
                          num_class=num_classes, dropout=0.1)
        elif ablation=='w_spatial_temporal':
            model = w_stmodel(time=32, height=8, width=8, channel=256,
                          out_time=16, out_channel=8, kernel_size=(2, 1, 1), stride=(2, 1, 1), padding=(0, 0, 0),
                          head_spatial_dim=256, head_spatial_cnt=4, scale_spatial_dim=2,
                          head_temporal_dim=512, head_temporal_cnt=4, scale_temporal_dim=2,
                          st_dim=512, head_st_dim=128, head_st_cnt=4, scale_st_dim=2,
            ...
            print('We only implemented w_spatial, w_temporal, w_spatial_temporal models.')
            raise NotImplementedError
...
if __name__ == "__main__":
    dataset = 'uex_class4'
    # token_size = 'type4-16×8×8×8'  # 'type2-32×8×8×8', 'type3-16×16×8×8' , 'type4-16×8×8×8'
    ablation = 'w_spatial_temporal'  # 'w_spatial', 'w_temporal' , 'w_spatial_temporal'
    optimizer_name = 'Adam'
    lr = 0.001
    weight_decay = 0.0001
    device = [1]
    ...
    save_epoch = 100
    processor(dataset=dataset, ablation=ablation, optimizer_name=optimizer_name, lr=lr, weight_decay=weight_decay, device_list=device,
